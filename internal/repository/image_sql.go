...
func (i ImageRepo) Save(ctx context.Context, image model.Image) error {
	...
	INSERT INTO images (sha , size, chunk_size , "status" , "created_at")
			VALUES ($1, $2, $3, $4,$5)`
	q := i.pgxPool.QueryRow(ctx, sqlStatement, image.Sha256, image.Size, image.ChunkSize, image.Status, time.Now().UTC())
	return q.Scan()
}

func (i ImageRepo) Update(ctx context.Context, image model.Image) error {
	sqlStatement := `
	update images set  status = $1  where sha = $2`
	_, err := i.pgxPool.Exec(ctx, sqlStatement, image.Status, image.Sha256)
	return err
}
